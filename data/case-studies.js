...
export const CASE_STUDIES = [
  ...
  },
  {
    ...
    title: 'A move-history list that never dropped a frame, no matter how long the match ran',
    hook: 'Language-free move history, built from icons alone — and it had to scale forever.',
    problem: "Kurukshetra's move-history feature showed every deploy, attack, and ability use as icons (no text, to sidestep language barriers) — but as a match went on, that list could grow indefinitely, mixing several differently-sized UI element types each with their own data and stats.",
    approach: "Built a highly optimized, smoothly scrolling dynamic list using object pooling across multiple mixed UI element types, recycling and rebinding elements instead of instantiating new ones as the history grew.",
    result: "Zero bugs reported in internal QA or production, and zero performance complaints — even in long-running matches with hundreds of entries.",
    tags: ['Object Pooling', 'Unity UI', 'Performance', 'Optimization'],
  },
  {
  ...
  {
    id: 'case-4', tag: 'GreyWake: Project Somnium',
    title: 'A save system that protects itself from you',
    hook: 'Plug-and-play saves, with guardrails against editor mistakes.',
    problem: "As the only programmer on a tiny indie team, a horror game needed a save/load system robust enough that designers and artists wouldn't need to understand it, and safe enough that accidental in-editor changes to data couldn't silently corrupt save-related state.",
    approach: "Built a plug-and-play save/load system with in-editor data protection, plus an auto-persistent runtime ID scheme for ScriptableObjects so references stay valid across saves without manual bookkeeping.",
    result: "A save system the rest of the small team can rely on without touching it themselves — freeing them to focus on content instead of save/load edge cases.",
    tags: ['Save Systems', 'ScriptableObjects', 'Editor Tooling', 'Solo Architecture'],
  },
  {
    id: 'case-5', tag: 'Omneity — Founder',
